...
def test_order_identity_and_complete_flow(datasets,monkeypatch,count):
    ...
    assert events==[(stage,d.name) for d in datasets for stage in ('profiling','semantic','planning','execution')]+[('entities',None),('findings',None)]
...
@pytest.mark.parametrize('stage',['profiling','semantic_understanding','analysis_planning',
                                 'analysis_execution','entities','correlation_planning','correlation_execution','findings'])
def test_failure_context_and_fail_fast(datasets,monkeypatch,stage):
    ...
    assert error.value.dataset is (None if stage.startswith('correlation') or stage in ('findings','entities') else datasets[0])
...
def synthetic_pipeline():
    ...
    return InvestigationPipeline(semantic_service=semantic,analysis_planner=planner,
        ...
        correlation_executor=CorrelationExecutionService(),finding_service=FindingService(ai),entity_service=EntityService())
